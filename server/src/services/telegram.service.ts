...
class TelegramService {
  ...
  private async ensureUserAndToken(msg: Message): Promise<{
    ...
    console.log('👤 [TELEGRAM][AUTH] ensureUserAndToken invoked', {
      chatId,
    ...
    try {
      console.log('🔍 [TELEGRAM][AUTH] Attempting login for chatId', chatId);
      ...
      if (existingUser) {
        ...
        console.log('✅ [TELEGRAM][AUTH] Existing user logged in', {
          chatId,
      ...
      console.warn(
        '⚠️ [TELEGRAM] Login attempt failed while ensuring user, will try signup:',
        error.message || error,
      );
    }

    try {
      console.log('🆕 [TELEGRAM][AUTH] Creating new user from Telegram chat', { chatId });
      const newUser = await authService.signup(signupPayload);
      console.log('🎉 [TELEGRAM][AUTH] Signup successful', {
        chatId,
        userId: newUser._id,
      });
      ...
      if (error.message?.toLowerCase().includes('already exists')) {
        console.warn('♻️ [TELEGRAM][AUTH] Signup reported existing user, retrying login', {
          chatId,
          error: error.message,
        });
        ...
        if (fallbackUser) {
          ...
          console.log('✅ [TELEGRAM][AUTH] Fallback login succeeded after signup conflict', {
            chatId,
  ...
  private setupBotHandlers(): void {
    ...
    this.bot.onText(/\/start/, async (msg: Message) => {
      ...
      try {
        ...
        console.log('✅ [TELEGRAM] User ensured via /start', {
          chatId,
          isNew,
        });
        const loginUrl = this.buildLoginUrl(chatId, token);
        console.log('🌐 [TELEGRAM] Login URL generated via /start', {
          chatId,
          loginUrl,
        });
        ...
        const intro = isNew
          ? `Hi ${firstName}! 🎉 Your AI Earn Bot account has been created automatically.`
        ...
        console.log('💬 [TELEGRAM] Sent /start welcome message', { chatId });
    ...
    this.bot.onText(/^(open|launch)$/i, async (msg: Message) => {
      ...
      try {
        ...
        await this.bot!.sendMessage(
          chatId,
          '🚀 Launching AI Earn Bot inside Telegram. Tap the button below if it does not open automatically.',
          {
            reply_markup: this.buildLaunchKeyboard(loginUrl),
          },
        );
